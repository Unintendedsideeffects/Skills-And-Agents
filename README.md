...
**New!** 🎉 Check out [Agent Manager](docs/AGENT_MANAGER.md) - a beautiful TUI for discovering, organizing, and managing your agents and skills across repositories!
...
See [Agent Manager Guide](docs/AGENT_MANAGER.md) for full documentation.
...
./scripts/install.sh install sre-code-reviewer project-owner-advisor
...
The repository includes a JSON schema (`agent-manager/schema.json`) that defines:
...
│   ├── GETTING_STARTED.md
│   └── QUICKSTART.md
├── agent-manager/             # Agent Manager app (Python package)
│   ├── pyproject.toml
...
5. **Schema compliance** - Validates against `agent-manager/schema.json`
