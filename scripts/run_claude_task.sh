...
show_help() {
  cat <<'EOF'
Usage: scripts/run_claude_task.sh [options]

Run Claude Code in non-interactive mode with timeout/retry guardrails.
...
Options:
  ...
  --fallback-model <name>     Fallback model for overload (default: haiku)
  --effort <low|medium|high>  Claude effort level (default: medium)
  --output-format <fmt>       text|json|stream-json (default: stream-json)
  --timeout <seconds>         Attempt timeout in seconds (default: 1200)
...
Examples:
  scripts/run_claude_task.sh --prompt "Reply with OK" --output-format text
  scripts/run_claude_task.sh --prompt-file /tmp/task.txt --timeout 1800 --retries 2
EOF
}

die() {
  echo "run_claude_task.sh: $*" >&2
  exit 1
}
...
is_retryable_failure() {
  local log_file="$1"
  rg -qi \
    "overloaded|capacity|rate limit|too many requests|429|temporar|timed out|timeout|network error|connection reset" \
    "$log_file"
}
...
while (( attempt <= max_attempts )); do
  ...
  if (( attempt < max_attempts )) && [[ -n "$retry_reason" ]]; then
    ...
    echo "run_claude_task.sh: attempt ${attempt}/${max_attempts} failed (${retry_reason}), retrying in ${sleep_for}s" >&2
    rm -f "$log_tmp"
    sleep "$sleep_for"
    attempt=$((attempt + 1))
    continue
